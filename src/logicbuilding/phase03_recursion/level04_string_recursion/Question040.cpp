...
int countConsonantsRecursive(string text, int index) {
if (index == text.size()) return 0;
char ch = text[index];
int add = isalpha(ch) && !isVowel(ch) ? 1 : 0;
return add + countConsonantsRecursive(text, index + 1);
}

int main() {
// Question 40: : Count consonants and vowels separately using recursion.
string text = "recursion";
cout << "Vowels = " << countVowelsRecursive(text, 0) << endl;
cout << "Consonants = " << countConsonantsRecursive(text, 0) << endl;
return 0;
}
